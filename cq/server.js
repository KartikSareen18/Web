...
app.set('view engine','ejs');
  ...
  var mongoDB = 'mongodb://localhost/myNewDB';
...
var  tagschema=new mongoose.Schema({
  ...
  var comm=mongoose.model('community',communityschema);
...
//      });
  ...
  {
     comm.find({ $or: [{ id : req.session.data._id },{join : {$in : [req.session.data._id] }},{asktojoin : {$in : [req.session.data._id] }}] }).exec(function(error,result){
        if(error)
        throw error;
        else {
            res.send(JSON.stringify(result));
...
app.get('/getsearch',function(req,res)
       {
    comm.find({ $and: [{ id : { $not : { $eq : req.session.data._id }}},{join : {$nin : [req.session.data._id] }},{asktojoin : {$nin : [req.session.data._id] }}] }).exec(function(error,result){
        if(error)
        throw error;
        else {
            res.send(JSON.stringify(result));
...
app.get('/getallcommunity',function(req,res)
  {
     var data=comm.find({}).exec(function(error,result)
                               {
         if(error)
             throw error;
         else
...
app.post('/pagination' , function (req , res)
{
    ...
    console.log(params);
    comm.find(query ,{},params, function (err , data)
        {
            if(err)
            {
                console.log(err);
                ...
                console.log(data);
                comm.countDocuments(function(err , count)
                {
                    if(err)
                    {
                        console.log(err);
...
app.get('/admincommunityprofile',function(req,res)
  {
      product.find({
          _id: req.session._id,
      })
...
app.get('/usercommunityprofile',function(req,res)
  {
      product.find({
          _id: req.session._id,
      })
...
app.get('/adminsetting',function(req,res)
  {
      product.find({
          _id: req.session._id,
      })
...
app.post('/updatecommunity',function(req,res)
       {
    console.log(req.body);
        comm.updateOne({"_id":req.body._id},{$set:{"communityname":req.body.communityname,"rule":req.body.rule}},function(error,result){
...
app.post('/userjoin',function(req,res)
       {
    console.log(req.body);
        comm.findOneAndUpdate({"_id":req.body._id},{$push:{join:req.session._id,member:"1"}},function(error,result){
...
app.post('/userasktojoin',function(req,res)
       {
    console.log(req.body);
        comm.findOneAndUpdate({"_id":req.body._id},{$push:{asktojoin:req.session._id}},function(error,result){
